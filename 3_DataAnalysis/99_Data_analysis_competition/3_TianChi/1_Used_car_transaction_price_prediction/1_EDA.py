...
# 并转换为PD时间格式
train_data['regDate3'] = train_data['regDate2'].map(lambda x: pd.to_datetime(x, format = "%Y") if(len(x) == 4) else pd.to_datetime(x, format = "%Y%m%d"))
# In[]:
# 新增regDate的标识列
...
# In[]:
# 发动机功率分桶
ccc["power_cut"], updown = pd.cut(ccc['power'], [-np.inf,100,200,300,400,500,600,np.inf], labels=[0,1,2,3,5,6,7], retbins=True)
