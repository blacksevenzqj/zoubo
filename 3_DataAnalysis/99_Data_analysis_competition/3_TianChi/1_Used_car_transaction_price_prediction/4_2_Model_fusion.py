...
1）结果层面的融合，这种是最常见的融合方法，其可行的融合方法也有很多，比如根据结果的得分进行加权融合，还可以做Log，exp处理等。在做结果融合的时候，有一个很重要的条件是（各个Level1的）模型（单独训练以及预测 或 交叉验证）结果的得分要比较近似，然后结果的差异要比较大（应该指的是特征层面），这样的结果融合往往有比较好的效果提升。
