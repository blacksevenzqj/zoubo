...
# 异常值检测
# power发动机功率：
box_more_index_power, hist_more_index_power = ft.outlier_detection(train_data_1, 'power', train_data_1, 'price', box_scale=3)
box_more_index_power_test, hist_more_index_power_test = ft.outlier_detection(test_data_1, 'power', box_scale=3)
# In[]:
print(len(box_more_index_power[0]), len(box_more_index_power[1]), len(hist_more_index_power[0]), len(hist_more_index_power[1]))
...
# In[]:
print(len(box_more_index_price[0]), len(box_more_index_price[1]), len(hist_more_index_price[0]), len(hist_more_index_price[1]))
# In[]:
# 删5倍标准差以外的数据
...
train_data_1.drop(['creatDate', 'regDate','regDate1','regDate2','regDate3','diff_day1', 'diff_day2'], axis=1, inplace=True)
# In[]:
...
test_data_1.drop(['creatDate', 'regDate','regDate1','regDate2','regDate3','diff_day1', 'diff_day2'], axis=1, inplace=True)


# In[]:
# 从邮编中提取城市信息，因为是德国的数据，所以参考德国的邮编，相当于加入了先验知识
train_data_1['city'] = train_data_1['regionCode'].map(lambda x : str(x)[:-3] if len(str(x)) == 4  else str(x)[:-2]  if len(str(x)) == 3 else str(x)[:-1]  if len(str(x)) == 2  else str(x))
print(train_data_1['city'].isnull().sum())
train_data_1[['regionCode', 'city']][0:10]
train_data_1.drop('regionCode', axis=1, inplace=True)
# In[]
test_data_1['city'] = test_data_1['regionCode'].map(lambda x : str(x)[:-3] if len(str(x)) == 4  else str(x)[:-2]  if len(str(x)) == 3 else str(x)[:-1]  if len(str(x)) == 2  else str(x))
...
# In[]:
# seller分类特征： 类别严重偏斜 0:149999 1:1 删除该特征 （根本不用看测试集了）
...
# In[]:
# offerType分类特征： 类别严重偏斜 0:150000 删除该特征 （根本不用看测试集了）
...
# In[]:
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'bodyType_amount':len, "bodyType_price_max":np.max, "bodyType_price_median":np.median, "bodyType_price_min":np.min, "bodyType_price_sum":np.sum, "bodyType_price_std":np.std, "bodyType_price_average":np.mean}
...
# In[]:
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'fuelType_amount':len, "fuelType_price_max":np.max, "fuelType_price_median":np.median, "fuelType_price_min":np.min, "fuelType_price_sum":np.sum, "fuelType_price_std":np.std, "fuelType_price_average":np.mean}
...
# In[]:
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'gearbox_amount':len, "gearbox_price_max":np.max, "gearbox_price_median":np.median, "gearbox_price_min":np.min, "gearbox_price_sum":np.sum, "gearbox_price_std":np.std, "gearbox_price_average":np.mean}
...
# In[]:
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'kilometer_amount':len, "kilometer_price_max":np.max, "kilometer_price_median":np.median, "kilometer_price_min":np.min, "kilometer_price_sum":np.sum, "kilometer_price_std":np.std, "kilometer_price_average":np.mean}
...
# In[]:
# 斯皮尔曼自动分箱： power发动机功率
data_spearmanr_power, cut_updown, bin_num_power, r_list_power = bt.spearmanr_auto_bins(train_data_2['power'], train_data_2['price'])
# In[]:
unique_label_power, counts_label_power, unique_dict_power = ft.category_quantity_statistics_all(data_spearmanr_power['Bucket'])
# In[]:
...
# 新增特征
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'power_cut_bin_amount':len, "power_cut_bin_price_max":np.max, "power_cut_bin_price_median":np.median, "power_cut_bin_price_min":np.min, "power_cut_bin_price_sum":np.sum, "power_cut_bin_price_std":np.std, "power_cut_bin_price_average":np.mean}
power_cut_bin_fe = tc.groupby_agg_oneCol(temp_data, ['power_cut_bin'], 'price', agg, as_index=False)


# In[]:
# 斯皮尔曼自动分箱： diff_day3使用时间
data_spearmanr_diff_day, cut_updown, bin_num_diff_day, r_list_diff_day = bt.spearmanr_auto_bins(train_data_2['diff_day3'], train_data_2['price'])
# In[]:
unique_label_diff_day, counts_label_diff_day, unique_dict_diff_day = ft.category_quantity_statistics_all(data_spearmanr_diff_day['Bucket'])
# In[]:
...
# 新增特征
temp_data = train_data_2[train_data_2['price'] > 0] # 数据 条件 提前做好
agg = {'diff_day_cut_bin_amount':len, "diff_day_cut_bin_price_max":np.max, "diff_day_cut_bin_price_median":np.median, "diff_day_cut_bin_price_min":np.min, "diff_day_cut_bin_price_sum":np.sum, "diff_day_cut_bin_price_std":np.std, "diff_day_cut_bin_price_average":np.mean}
...
# In[]:
del_skew_columns = ['fuelType_price_max','brand_price_min','fuelType_price_min','fuelType_price_sum','kilometer_price_min','kilometer_price_max']
