...
ft.set_file_path(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\100_Data_analysis_competition\2_DataFountain\2_Immediate-AI-Global-Challenger-Competition\data\AI_risk_train_V3.0_new")
...
# In[]:
# 导入字段类型检测
print(train_auth["id_card"].dtypes, train_auth.loc[0,"id_card"], type(train_auth.loc[0,"id_card"]), train_auth.loc[0,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[1,"id_card"], type(train_auth.loc[1,"id_card"]), train_auth.loc[1,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[3,"id_card"], type(train_auth.loc[3,"id_card"]), train_auth.loc[3,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[5,"id_card"], type(train_auth.loc[5,"id_card"]), train_auth.loc[5,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[7,"id_card"], type(train_auth.loc[7,"id_card"]), train_auth.loc[7,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[8,"id_card"], type(train_auth.loc[8,"id_card"]), train_auth.loc[8,"id_card"] is np.nan)
print(train_auth["id_card"].dtypes, train_auth.loc[14,"id_card"], type(train_auth.loc[14,"id_card"]), train_auth.loc[14,"id_card"] is np.nan)
# In[]:
print(train_auth["auth_time"].dtypes, train_auth.loc[0,"auth_time"], type(train_auth.loc[0,"auth_time"]), train_auth.loc[0,"auth_time"] is np.nan, train_auth.loc[0,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[1,"auth_time"], type(train_auth.loc[1,"auth_time"]), train_auth.loc[1,"auth_time"] is np.nan, train_auth.loc[1,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[5,"auth_time"], type(train_auth.loc[5,"auth_time"]), train_auth.loc[5,"auth_time"] is np.nan, train_auth.loc[5,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[7,"auth_time"], type(train_auth.loc[7,"auth_time"]), train_auth.loc[7,"auth_time"] is np.nan, train_auth.loc[7,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[8,"auth_time"], type(train_auth.loc[8,"auth_time"]), train_auth.loc[8,"auth_time"] is np.nan, train_auth.loc[8,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[15,"auth_time"], type(train_auth.loc[15,"auth_time"]), train_auth.loc[15,"auth_time"] is np.nan, train_auth.loc[15,"auth_time"] is pd.lib.NaT)
print(train_auth["auth_time"].dtypes, train_auth.loc[17,"auth_time"], type(train_auth.loc[17,"auth_time"]), train_auth.loc[17,"auth_time"] is np.nan, train_auth.loc[17,"auth_time"] is pd.lib.NaT)
...
print(train_auth["auth_time"].dtypes, train_auth.loc[29,"auth_time"], type(train_auth.loc[29,"auth_time"]), train_auth.loc[29,"auth_time"] is np.nan, train_auth.loc[29,"auth_time"] is pd.lib.NaT)
# In[]:
...
# In[]:
# 是否认证时间在借贷时间前
tmp_train_auth['is_auth_time_before_borrow_time'] = tmp_train_auth.apply(lambda x:0 if (x['is_auth_time_authtable'] == 0) else ( 1 if x['auth_time'] < x['appl_sbm_tm'] else 0),axis=1)
# In[]:
# 是否认证时间在借贷时间后
tmp_train_auth['is_auth_time_after_borrow_time'] = tmp_train_auth.apply(lambda x:0 if (x['is_auth_time_authtable'] == 0) else ( 1 if x['auth_time'] > x['appl_sbm_tm'] else 0),axis=1)
# In[]:
# 认证时间在借贷时间前多少天
tmp_train_auth['auth_time_before_borrow_time_days'] = tmp_train_auth.apply(lambda x:0 if (x['is_auth_time_before_borrow_time'] == 0) else (x['appl_sbm_tm'] - x['auth_time']).days,axis=1)
# In[]:
# 认证时间在借贷时间后多少天
tmp_train_auth['auth_time_after_borrow_time_days'] = tmp_train_auth.apply(lambda x:0 if (x['is_auth_time_after_borrow_time'] == 0) else (x['auth_time'] - x['appl_sbm_tm']).days,axis=1)
...
# In[]:
# 导入字段类型检测
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[0,"tail_num"], type(train_bankcard.loc[0,"tail_num"]), train_bankcard.loc[0,"tail_num"] is np.nan, isnan(train_bankcard.loc[0,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[1,"tail_num"], type(train_bankcard.loc[1,"tail_num"]), train_bankcard.loc[1,"tail_num"] is np.nan, isnan(train_bankcard.loc[1,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[2,"tail_num"], type(train_bankcard.loc[2,"tail_num"]), train_bankcard.loc[2,"tail_num"] is np.nan, isnan(train_bankcard.loc[2,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[3,"tail_num"], type(train_bankcard.loc[3,"tail_num"]), train_bankcard.loc[3,"tail_num"] is np.nan, isnan(train_bankcard.loc[3,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[4,"tail_num"], type(train_bankcard.loc[4,"tail_num"]), train_bankcard.loc[4,"tail_num"] is np.nan, isnan(train_bankcard.loc[4,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[5,"tail_num"], type(train_bankcard.loc[5,"tail_num"]), train_bankcard.loc[5,"tail_num"] is np.nan, isnan(train_bankcard.loc[5,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[6,"tail_num"], type(train_bankcard.loc[6,"tail_num"]), train_bankcard.loc[6,"tail_num"] is np.nan, isnan(train_bankcard.loc[6,"tail_num"]))
print(train_bankcard["tail_num"].dtypes, train_bankcard.loc[7,"tail_num"], type(train_bankcard.loc[7,"tail_num"]), train_bankcard.loc[7,"tail_num"] is np.nan, isnan(train_bankcard.loc[7,"tail_num"]))
# In[]:
"增加特征"
# 3.1、唯一用户 有几家不同银行的卡
...
bank_phone_num_setlen = tc.groupby_agg_oneCol(train_bankcard, ["id"], "phone", agg, as_index=False) # 速度近10倍于groupby_apply
...
# 3.4、分别有 几张信用卡、储蓄卡
agg = {'数量':len}
train_bankcard_card_type_num = tc.groupby_agg_oneCol(train_bankcard, ["id","card_type"], "bank_name", agg, as_index=False)
# In[]:
...
# In[]:
# 信用卡
train_target_bankcard_NewF = pd.merge(train_target_bankcard_NewF, credit_card[["id","credit_card"]], on = ['id'], how='left')
# In[]:
# 储蓄卡
train_target_bankcard_NewF = pd.merge(train_target_bankcard_NewF, debit_card[["id","debit_card"]], on = ['id'], how='left')
# In[]:
# 有几个不同电话号码
...
# In[]:
# 导入字段类型检测
print(train_credit["quota"].dtypes, train_credit.loc[0,"quota"], type(train_credit.loc[0,"quota"]), train_credit.loc[0,"quota"] is np.nan, isnan(train_credit.loc[0,"quota"]))
print(train_credit["quota"].dtypes, train_credit.loc[1,"quota"], type(train_credit.loc[1,"quota"]), train_credit.loc[1,"quota"] is np.nan, isnan(train_credit.loc[1,"quota"]))
print(train_credit["quota"].dtypes, train_credit.loc[2,"quota"], type(train_credit.loc[2,"quota"]), train_credit.loc[2,"quota"] is np.nan, isnan(train_credit.loc[2,"quota"]))
print(train_credit["quota"].dtypes, train_credit.loc[43,"quota"], type(train_credit.loc[43,"quota"]), train_credit.loc[43,"quota"] is np.nan, isnan(train_credit.loc[43,"quota"]))
# In[]:
...
# In[]:
print(train_order["amt_order"].dtypes, train_order.loc[0,'amt_order'], type(train_order.loc[0,'amt_order']), train_order.loc[0,'amt_order'] is np.nan, isnan(train_order.loc[0,"amt_order"]))
print(train_order["amt_order"].dtypes, train_order.loc[8,'amt_order'], type(train_order.loc[8,'amt_order']), train_order.loc[8,'amt_order'] is np.nan, isnan(train_order.loc[8,"amt_order"]))
print(train_order["unit_price"].dtypes, train_order.loc[0,'unit_price'], type(train_order.loc[0,'unit_price']), train_order.loc[0,'unit_price'] is np.nan, isnan(train_order.loc[0,"unit_price"]))
print(train_order["unit_price"].dtypes, train_order.loc[17,'unit_price'], type(train_order.loc[17,'unit_price']), train_order.loc[17,'unit_price'] is np.nan, isnan(train_order.loc[17,"unit_price"]))
# In[]:
print(train_order["time_order"].dtypes, train_order.loc[0,"time_order"], type(train_order.loc[0,"time_order"]), train_order.loc[0,"time_order"] is np.nan, train_order.loc[0,"time_order"] is pd.lib.NaT)
print(train_order["time_order"].dtypes, train_order.loc[8,"time_order"], type(train_order.loc[8,"time_order"]), train_order.loc[8,"time_order"] is np.nan, train_order.loc[8,"time_order"] is pd.lib.NaT)
# In[]:
...
before_appl_sbm_tm_howmany = tc.groupby_agg_oneCol(tmp_train_order_before_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
# In[]:
# 借贷时间后有多少次购买
...
after_appl_sbm_tm_howmany = tc.groupby_agg_oneCol(tmp_train_order_after_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
...
before_appl_sbm_tm_money_mean = tc.groupby_agg_oneCol(tmp_train_order_before_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
# In[]:
# 借贷时间后购买金额均值
...
after_appl_sbm_tm_money_mean = tc.groupby_agg_oneCol(tmp_train_order_after_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
...
before_appl_sbm_tm_money_max = tc.groupby_agg_oneCol(tmp_train_order_before_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
# In[]:
# 借贷时间前购买金额最小值
...
before_appl_sbm_tm_money_min = tc.groupby_agg_oneCol(tmp_train_order_before_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
...
after_appl_sbm_tm_money_max = tc.groupby_agg_oneCol(tmp_train_order_after_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
# In[]:
# 借贷时间后购买金额最小值
...
after_appl_sbm_tm_money_min = tc.groupby_agg_oneCol(tmp_train_order_after_appl_sbm_tm, ["id"], "amt_order", agg, as_index=False)
diff = pd.Timestamp.now() - start_time
print(diff, type(diff), diff.days, diff.seconds, diff.total_seconds())
...
# In[]:
# 导入字段类型检测
print(train_recieve["region"].dtypes, train_recieve.loc[0,"region"], type(train_recieve.loc[0,"region"]), train_recieve.loc[0,"region"] is np.nan)
print(train_recieve["region"].dtypes, train_recieve.loc[1,"region"], type(train_recieve.loc[1,"region"]), train_recieve.loc[1,"region"] is np.nan)
print(train_recieve["region"].dtypes, train_recieve.loc[25327,"region"], type(train_recieve.loc[25327,"region"]), train_recieve.loc[25327,"region"] is np.nan)
# In[]:
print(train_recieve["addr_id"].dtypes, train_recieve.loc[0,"addr_id"], type(train_recieve.loc[0,"addr_id"]), train_recieve.loc[0,"addr_id"] is np.nan, isnan(train_recieve.loc[0,"addr_id"]))
print(train_recieve["addr_id"].dtypes, train_recieve.loc[1,"addr_id"], type(train_recieve.loc[1,"addr_id"]), train_recieve.loc[1,"addr_id"] is np.nan, isnan(train_recieve.loc[1,"addr_id"]))
print(train_recieve["addr_id"].dtypes, train_recieve.loc[25327,"addr_id"], type(train_recieve.loc[25327,"addr_id"]), train_recieve.loc[25327,"addr_id"] is np.nan, isnan(train_recieve.loc[25327,"addr_id"]))
# In[]:
...
recieve_score_mean = tmp_recieve_target.groupby(by= ['id'], as_index = False)['recieve_default_rate'].agg({'recieve_default_rate_mean':np.mean})
...
# In[]:
# 查询出留下的正常数据
a = train_user1[train_user1['birthday'].map(lambda x: re.match("^(19|20)\d{2}-\d{1,2}-\d{1,2}", str(x)) != None)]["birthday"].index.tolist()
...
tmp_train_user = pd.merge(tmp_train_user, ccc, on = ['sex'], how='left') # train_user_info行维度 与 train_target相同，不需要再groupby
...
# 同样可以看看模型被保存到了哪里
joblib.dump(model,r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\100_Data_analysis_competition\2_DataFountain\2_Immediate-AI-Global-Challenger-Competition\MyLearn\train_data.dat")
# In[]:
loaded_model = joblib.load(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\100_Data_analysis_competition\2_DataFountain\2_Immediate-AI-Global-Challenger-Competition\MyLearn\train_data.dat")
# In[]:
