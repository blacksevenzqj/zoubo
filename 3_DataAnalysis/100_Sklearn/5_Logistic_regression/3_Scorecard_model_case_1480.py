...
print('训练集数据中，好客户数量为：%i,坏客户数量为：%i,坏客户所占比例为：%.2f%%' % (
goodnum, badnum, (badnum / train_data['SeriousDlqin2yrs'].count()) * 100))
...
print(train_data[train_data['NumberOfTime30-59DaysPastDueNotWorse'] > 13][
          'NumberOfTime30-59DaysPastDueNotWorse'].value_counts())
print('----------------------')
# 这里可以看出逾期30-59天次数大于13次的有269条，大于80次的也是269条，说明这些是异常值，应该删除
print(train_data[train_data['NumberOfTime60-89DaysPastDueNotWorse'] > 13][
...
# 查看缺失比例
x = (train_data['SeriousDlqin2yrs'].count() - train_data['NumberOfDependents'].count()) / train_data[
...
train_data.loc[(train_data['RevolvingUtilizationOfUnsecuredLines'] > 1) | (train_data['age'] < 18) | (
        train_data['NumberOfTime30-59DaysPastDueNotWorse'] > 80) | (
                       ...
                       train_data['NumberOfTimes90DaysLate'] > 80) | (train_data['NumberRealEstateLoansOrLines'] > 50)]
...
、
使用 斯皮尔曼系数（“2、Spearman运用于分箱”） 计算 连续自变量X 与 二分类因变量Y 之间的相似度
'''


def mono_bin(Y, X, n=10):  # X为待分箱的连续变量，Y为target二分类变量,n为分箱数量
    r = 0  # 设定斯皮尔曼 初始值
    badnum = Y.sum()  # 计算坏样本数
...
df_all_corr_abs.rename(columns={"level_0": "Feature_1", "level_1": "Feature_2", 0: 'Correlation_Coefficient'},
                       inplace=True)
print(df_all_corr_abs[
          (df_all_corr_abs["Feature_1"] != 'SeriousDlqin2yrs') & (df_all_corr_abs['Feature_2'] == 'SeriousDlqin2yrs')])
print()
# 皮尔森相似度 排序
