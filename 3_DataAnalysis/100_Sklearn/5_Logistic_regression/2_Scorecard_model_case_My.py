...
# 注意zip会按照最短列来进行结合
print(num_bins)
0 (21.0, 28.0, 4243, 6067)
1 (28.0, 31.00987362029104, 3571, 5620)
'''
...
# 0 返回 chi2 值，1 返回 p 值。
print([x1, x2])  # [(4243, 6067), (3571, 5620)]
...
'''
χ2分布有两个重要用途：
1、检验拟合优度：检验一组给定数据与指定分布的吻合程度。其基本思想是 根据 样本观测数据 推断 总体的分布 与 期望分布 是否有显著性差异。
2、检验两个分类变量的独立性，推断两个分类变量 独立 或者 相关。

显著性水平α（右尾拒绝域概率）：5%、1%   →   置信度：95%、99%
自由度v（或k）：(行数-1)*(列数-1)
根据 显著性水平α（右尾拒绝域概率） 和  自由度v（或k）查表得到 卡方阈值。
检验结果方式：
方式1、计算出来的  卡方值  与  卡方阈值  进行比较。
方式2、计算出来的  概率值p  与  显著性水平α（右尾拒绝域概率） 进行比较。

原假设： 分箱后的 两个小箱体 没有区别。即：1、观察频数与期望频数没有差异； 2、两个分类变量（小箱体 和 是否违约）相互独立、不相关
卡方值越大（大于卡方阈值） → p值越小（小于显著性水平α） → 越在拒绝域之内 → 拒绝原假设 → 得到： 1、观测值与期望值不相似； 2、箱体 和 是否违约 不互相独立、相关 → 不合箱
卡方值越小（小于卡方阈值） → p值越大（大于显著性水平α） → 越在拒绝域之外 → 接受原假设 → 得到： 1、观测值与期望值相似； 2、箱体 和 是否违约 互相独立、不相关 → 合箱
注意： 合箱的目的 直到 箱体 和 是否违约 不互相独立、相关 为止（也就是 一直合箱 直到 拒绝原假设 为止）。这样 连续变量分箱后 才对 二分类因变量Y 有贡献。
也就是在 接受原假设时 要进行合箱： 因为 小箱体 和 是否违约 相互独立、不相关，对 二分类因变量Y 没有贡献。
'''
for i in range(len(num_bins_) - 1):
    x1 = num_bins_[i][2:]
    x2 = num_bins_[i + 1][2:]
    pv = scipy.stats.chi2_contingency([x1, x2])[1]  # p值
    chi2 = scipy.stats.chi2_contingency([x1, x2])[0]  # 计算卡方值
    print(str(i) + '→' + str(i + 1), [x1, x2], pv, chi2)
    print('-' * 30)
'''
0→1 [(4243, 6067), (3571, 5620)] 0.0011199672373298991 10.61792383028901
------------------------------
1→2 [(3571, 5620), (4075, 6664)] 0.194076881463525 1.686394913356514
------------------------------
2→3 [(4075, 6664), (2908, 5854)] 6.103261608155979e-12 47.29612940200499
------------------------------
3→4 [(2908, 5854), (5182, 5875)] 3.4172928415567135e-84 377.98490464733965
------------------------------
4→5 [(5182, 5875), (3956, 5518)] 2.296752227208079e-13 53.732980460812136
------------------------------
5→6 [(3956, 5518), (4002, 5770)] 0.2645165287602904 1.244963903558193
------------------------------
6→7 [(4002, 5770), (4389, 5736)] 0.000662277302579131 11.592236102321618
------------------------------
7→8 [(4389, 5736), (2419, 5905)] 5.8127060874327644e-89 399.89230566157534
------------------------------
8→9 [(2419, 5905), (4813, 4937)] 1.505227552319497e-169 770.3548514690426
------------------------------
9→10 [(4813, 4937), (4900, 4850)] 0.21805128503017898 1.5171500536849283
------------------------------
10→11 [(4900, 4850), (4728, 5980)] 2.7821365055578096e-18 76.03831581663697
------------------------------
11→12 [(4728, 5980), (4681, 5389)] 0.000781899218579815 11.283715286939966
------------------------------
12→13 [(4681, 5389), (4677, 4491)] 3.7563314312416155e-10 39.235575824593724
------------------------------
13→14 [(4677, 4491), (4483, 4573)] 0.042828232238705306 4.102169168951121
------------------------------
14→15 [(4483, 4573), (6583, 3985)] 2.1254533192735614e-72 323.82636383596173
------------------------------
15→16 [(6583, 3985), (6968, 3733)] 1.974786079853086e-05 18.21345687356962
------------------------------
16→17 [(6968, 3733), (6623, 2684)] 7.248929345022201e-20 83.2449822104428
------------------------------
17→18 [(6623, 2684), (6753, 2106)] 1.0841325638795456e-14 59.737103641946604
------------------------------
18→19 [(6753, 2106), (7737, 1580)] 4.088241715184003e-30 130.00462401682145
'''
# In[]:
