...
sns.distplot(data['NumberOfOpenCreditLinesAndLoans'][data["SeriousDlqin2yrs"] == 1], bins=100, color='red', ax=axes[1][0])
sns.distplot(data['NumberOfOpenCreditLinesAndLoans'][data["SeriousDlqin2yrs"] == 0], bins=100, color='blue', ax=axes[1][0])
...
model_data.to_csv(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\101_Sklearn\5_Logistic_regression\model_data.csv") # 训练数据
vali_data.to_csv(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\101_Sklearn\5_Logistic_regression\vali_data.csv") # 验证数据
# In[]:
...
#    print(model_data['SeriousDlqin2yrs'].astype('int64').groupby(model_data['cut']).agg(['count', 'mean']))
    ...
    pvlist.append(scipy.stats.chi2_contingency(bins_stats_crosstab.iloc[:-1, :-1])[1])
    index.append("x" + str(i+1))
