...
原假设： 分箱后的 两个小箱体 没有区别。即：1、观察频数与期望频数没有差异； 2、两个分类变量（小箱体 和 是否违约）相互独立不相关）
卡方值越大（大于卡方阈值） → p值越小（小于显著性水平α） → 越在拒绝域之内 → 拒绝原假设 → 得到： 1、观测值与期望值不相似； 2、箱体 和 是否违约 不互相独立 → 不可以合箱
卡方值越小（小于卡方阈值） → p值越大（大于显著性水平α） → 越在拒绝域之外 → 接受原假设 → 得到： 1、观测值与期望值相似； 2、箱体 和 是否违约 互相独立 → 可以合箱
'''
for i in range(len(num_bins_) - 1):
    x1 = num_bins_[i][2:]
    x2 = num_bins_[i + 1][2:]
