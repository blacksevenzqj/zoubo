...
#        tprs.append(TPR(y_true, my_predict, state)) # recalls
    ...
    print('max(TPR-FPR) = %.4f, 最大阈值 = %.4f, 召回率/TPR = %.4f, FPR = %.4f' % (abs(np.array(recalls) - np.array(fprs)).max(), thresholds[maxKsIndex], recalls[maxKsIndex], fprs[maxKsIndex]))
...
def ComprehensiveIndicatorFigure(y_true, decision_scores, axe, state=1):
    ...
    '''
    thresholds, precisions, recalls, fprs, f1Scores, recallsValue, fprsValue, maxKsThresholds, maxKsValue, maxF1Thresholds, maxF1ScoresValue, diffValue, defPValue, defRValue, defFValue, Ks_PValue, Ks_f1Value = ComprehensiveIndicator(y_true, decision_scores, state)
    ...
    axe[0].plot(thresholds, f1Scores, color='green', label='F1阈值 = %.4f，F1 = %.4f' % (maxF1Thresholds, maxF1ScoresValue))
    axe[0].plot(thresholds, fprs, color='pink', label='FPR')
    axe[0].scatter(0,defFValue,c='#FFC0CB',s=30,cmap="rainbow") # 粉色
    axe[0].plot((maxKsThresholds,maxKsThresholds), (recallsValue,fprsValue), c='r', lw=1.5, ls='--', alpha=0.7, label='KS阈值 = %.4f，KS = %.4f' % (maxKsThresholds, maxKsValue))
    axe[0].plot((maxKsThresholds,maxF1Thresholds), (maxF1ScoresValue,maxF1ScoresValue), c='purple', lw=1.5, ls='-', alpha=0.7, label='(F1-KS)的阈值差 = %.4f' % diffValue)
...
def ComprehensiveIndicatorSkLib(y_true, decision_scores):
    ...
    print('max(TPR-FPR) = %.4f, 最大阈值 = %.4f, 召回率/TPR = %.4f, FPR = %.4f' % (abs(np.array(tprs2) - np.array(fprs2)).max(), thresholds2[maxindex_auto], tprs2[maxindex_auto], fprs2[maxindex_auto]))
...
def ComprehensiveIndicatorSkLibFigure(y_true, decision_scores, axe):
    ...
    thresholds2, tprs2, fprs2, recallsValue_auto, fprsValue_auto, maxKsThresholds_auto, maxKsValue_auto, rocAucScore, defRValue, defFValue = ComprehensiveIndicatorSkLib(y_true, decision_scores)
    axe[0].plot(thresholds2, tprs2, color='black', label='召回率/TPR')
    axe[0].plot(thresholds2, fprs2, color='pink', label='FPR')
    axe[0].plot((maxKsThresholds_auto,maxKsThresholds_auto), (recallsValue_auto,fprsValue_auto), c='r', lw=1.5, ls='--', alpha=0.7, label='KS = %.4f，KS阈值 = %.4f' % (maxKsValue_auto, maxKsThresholds_auto))
    axe[0].scatter(0,defRValue,c='#000000',s=30,cmap="rainbow") # 黑色
    axe[0].scatter(0,defFValue,c='#FFC0CB',s=30,cmap="rainbow") # 粉色
    ...
    axe[0].text(0, defRValue + 0.03, '默认阈值0：%.4f' % defRValue, ha='center', va='bottom') 
    axe[0].text(0, defFValue - 0.03, '默认阈值0：%.4f' % defFValue, ha='center', va='bottom')
    axe[0].text(maxKsThresholds_auto, recallsValue_auto + 0.01, '%.4f' % recallsValue_auto, ha='center', va='bottom') # fontsize=8
    ...
    axe[1].plot((fprsValue_auto,fprsValue_auto), (recallsValue_auto,fprsValue_auto), c='r', lw=1.5, ls='--', alpha=0.7)
    axe[1].set_xlabel('FPR')  # x轴标签
    axe[1].set_ylabel('TPR') # y轴标签
    axe[1].grid(b=True)
    axe[1].legend(loc='lower right', fancybox=True, framealpha=0.8, fontsize=14)
    axe[1].set_title('自动-ROC曲线和AUC值') # fontsize=17
    axe[1].text(fprsValue_auto, recallsValue_auto + 0.01, '%.4f' % recallsValue_auto, ha='center', va='bottom') # fontsize=8
    axe[1].text(fprsValue_auto, fprsValue_auto - 0.05, '%.4f' % fprsValue_auto, ha='center', va='bottom')
    axe[1].text(fprsValue_auto + 0.05, (fprsValue_auto + recallsValue_auto)/2, '%.4f' % maxKsValue_auto, ha='center', va='bottom')
