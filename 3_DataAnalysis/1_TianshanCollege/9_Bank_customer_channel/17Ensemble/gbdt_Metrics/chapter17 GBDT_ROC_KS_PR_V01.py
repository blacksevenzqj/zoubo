...
import os
os.chdir(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\TianshanCollege\9_Bank_customer_channel\17Ensemble\gbdt_Metrics")
...
def plot_result(Y, pred, pred_proba):
    ...
    plt.show()
    # 输出roc曲线
    skplt.metrics.plot_roc_curve(Y, pred_proba,curves=('each_class')) # 对分类类别 分别做曲线
    plt.show()
    # 输出pr曲线
...
test_est_probas = clfcv.predict_proba(test_data)
plot_result(test_target, test_est, test_est_probas)
