...
print(pca.explained_variance_)  # 建议保留2个主成分   [1.60786876 1.00252275 0.7339482 ]
# explained_variance_ratio_： 解释方差占比（累计解释方差占比 自己手动加）
print(pca.explained_variance_ratio_)  # 建议保留3个主成分   [0.40196317 0.25062818 0.18348521]
# 因 0.40196317 + 0.25062818 + 0.18348521 = 0.83607656 才满足 80%-90%的要求（也有说70%就够了的），所以选3个主成分。

# %%
pca = PCA(n_components=3).fit(data)  # 综上分析,确定保留3个主成分
# fit_transform 得到降维后的数据：维度为 [100000, 3] 的 100000行数据，每行数据3个特征。
...
print(data[0:10])
print(data[:, 1])
# 第一个主成分 的 特征权重 差不多高，后3个稍高；
# 第二个主成分 的 第1个 特征权重 高，其余均低；
# 第三个主成分 的 第4个 特征权重 低，其余差不多。

# 因子旋转前的 三个主成分作散点图，需要做成3维图显示。
...
# In[8]:
# 保留3个主成分，上面PCA已算出
fa.find_comps_to_retain(method='top_n', num_keep=3)  # num_keep 保留主成分个数
...
print(fa_score.iloc[0:10])

# 第三步：根据因子得分进行数据分析
# In[25]:
a = fa_score.rename(columns={0: "自动交易", 1: "柜台交易", 2: "有偿服务次数"})
