...
def get_sample(df, sampling="simple_random", k=1, stratified_col=None):
    ...
    if k <= 0:
        ...
        if sampling is "stratified":
            alln=k*df.groupby(by=stratified_col)[stratified_col[0]].count().count() # 每层抽k个 * 层数（分类类别数）
            #alln=k*df[stratified_col].value_counts().count() 
            if alln >= len_df: # 如果 每层抽k个 * 层数（分类类别数） >= 总样本数 则报错
...
#%%
import pandas as pd
clients = pd.read_csv(r'E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\TianshanCollege\3_CreditCard_analysis\5Preprocessing\clients.csv',encoding = 'gbk')
...
#%%2分层抽样
#%%分层抽样-按每层数量取
#strn=get_sample(clients, sampling="stratified", k=2, stratified_col=['district_id','sex'])
stratified_col=['district_id']
a = clients.groupby(by=stratified_col)['sex'].count().count()
a
