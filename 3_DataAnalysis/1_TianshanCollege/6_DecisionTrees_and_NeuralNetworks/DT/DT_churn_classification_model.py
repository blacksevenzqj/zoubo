...
corrmatrix = churn.corr(
    method='spearman')  # spearman相关系数矩阵，可选pearson相关系数，目前仅支持这两种,函数自动排除 category 类型（我猜只是排除字符串类型，数字作为分类标识并不排除）
...
# 2.1.1、使用 SelectKBest 方法直接选出 x 个特征：
...
# 2.2.1、还是使用sklearn的包单独对 自变量prom 与 因变量churn 进行 自动卡方检验：
...
print(w00 + w01 + w10 + w11)  # 1.3315217118042797
'''
2.2.3.1、自由度 v=1（chi2函数：每个特征 分别与 churn['churn'] 进行卡方检验（所以 自由度v = (2-1) * (2-1) = 1）
...
model_data = churn[
    ['subscriberID', 'churn', 'gender', 'edu_class', 'feton', 'duration_bins', 'call_10086', 'AGE']]  # 第二可选方案
model_data.head()
...
train_data, test_data, train_target, test_target = train_test_split(data, target, test_size=0.4, train_size=0.6,
