...
os.chdir(r"E:\soft\Anaconda\Anaconda_Python3.6_code\data_analysis\TianshanCollege\7_BankAnti-fraud_model\16Imbalance")
...
from imblearn.combine import SMOTETomek # 联合抽样
...
# In[7]:
y_ros.sum(), y_sos.sum(), y_kos.sum() # 求出过采样后，=1的样本量，差不多就是总量的一半。
...
# In[10]:
# 2、采用改变样本权重的方法
# 'class_weight':[{0:1, 1:5}, {0:1, 1:10}] 表示 分类为0或1，权重比例为1:5
# 在计算指标时生效： 1类样本10, 0类样本100。 样本比为1:10。 当1类预测错误时，要乘以10的权重； 当1类预测正确时，不动作。
